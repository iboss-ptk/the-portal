...
void main()
{
    ...
    float r=distance(vec2(uv.x,uv.y),vec2(.5));
    
    vUv=uv;
    // vUv=vec2(vUv.x*r.x,vUv.y*r.y);
    // vUv=rotateUV(vUv,uTime);
    // vUv=vec2(r*2.*cos(uTime+vUv.x),r*2.*sin(uTime+vUv.y));
    
    // float effectRadius=.5;
    // float effectAngle=2.*PI;
    
    // float mid=.5;
    // float angle=atan(uv.x-mid,uv.y-mid)*r;
    // float angleOffset=(1./(5.*r))+uTime*.5;
    // angle+=angleOffset;
    // angle=mod(angle,.2);
    // vUv.x=cos(angle)*(r);
    // vUv.y=sin(angle)*(r);
