...
void main()
{
    ...
    // gl_FragColor=vec4(vec3(1.-vDebug),1.);
    
    // float strength=step(.8,mod(vUv.x*100.,1.));
    // strength*=step(.8,mod(vUv.y*100.,1.));
    // gl_FragColor=vec4(strength);
    
    // gl_FragColor=vec4(max(abs(vUv.x),abs(vUv.y)));
